...
enum SectionData: Hashable {
    ...
    // This is needed for differentiate hash value
    static var allMoviesForList : [SectionData] = {
        return  originalSampleMovies.map { movie in
            var movie = movie
            movie.id = -1
            return SectionData.movie(movie)
        }
    }()
    
    static func getGenreNames(genreIds: [Int])->String{
         return genreIds.reduce("") { (str, genreId) -> String in
