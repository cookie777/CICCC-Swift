...
class ViewController: UIViewController {
  ...
  lazy var stackView: UIStackView = {
    let sv = HorizontalStackView(arrangedSubviews: imageViewsArray,
  ...
  var nvButton: UIButton = {
    ...
    bt.addTarget(self, action: #selector(nvBarButtonPressed(_:)),
      for: .touchUpInside)
    return bt
  }()
  ...
  override func viewDidLoad() {
    ...
    nvBarView.anchors(topAnchor: nil, leadingAnchor: view.leadingAnchor,
      trailingAnchor: view.trailingAnchor, bottomAnchor: view.topAnchor,
      padding: .init(top: nil, left: 0, bottom: -100, right: 0))

    view.addSubview(nvButton)
    nvButton.anchors(topAnchor: view.topAnchor, leadingAnchor: nil,
      trailingAnchor: nvBarView.trailingAnchor, bottomAnchor: nil,
      padding: .init(top: 40, left: nil, bottom: nil, right: 32))


    nvBarView.addSubview(nvLabel)
    nvLabel.centerXin(nvBarView)
    nvLabelYConstraint = nvLabel.bottomAnchor.constraint(
      equalTo: nvBarView.bottomAnchor, constant: -16)
    nvLabelYConstraint.identifier = "nvLabelY"
    nvLabelYConstraint.isActive = true


    nvBarView.addSubview(stackView)
    stackView.anchors(topAnchor: nil, leadingAnchor: nvBarView.leadingAnchor,
      trailingAnchor: nvBarView.trailingAnchor,
      bottomAnchor: nvBarView.bottomAnchor)

    view.addSubview(tableView)
    tableView.anchors(topAnchor: nvBarView.bottomAnchor,
  ...
  @objc func nvBarButtonPressed(_ sender: UIButton) {
    ...
    UIView.animate(withDuration: 2, delay: 0, usingSpringWithDamping: 0.1,
  ...
  @objc func imageTapped(_ sender: UIGestureRecognizer) {
    if let item = (sender.view as? UIImageViewWithName)?.imageName {
      ...
      tableView.insertRows(at: [IndexPath(row: items.count - 1, section: 0)],
        with: .none)
    }
  }
